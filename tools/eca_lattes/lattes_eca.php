<?php
  ...
  while ($row = $stmt->fetch()) {
    ...
    if (!is_null($row["nomabvset"])) {
      $output = shell_exec('curl -X POST -F "file=@'.__DIR__.'/curriculo.xml" -F "codpes='.$row["codpes"].'" -F "unidade=ECA" -F "tag='.trim($row["nomabvset"]).'" -F "tipvin='.$row["tipvinext"].'" http://tecbib.com/coletaprodeca/lattes_xml_to_elastic.php');
    } else {
      $output = shell_exec('curl -X POST -F "file=@'.__DIR__.'/curriculo.xml" -F "codpes='.$row["codpes"].'" -F "unidade=ECA" -F "tag='.trim($row["nomcur"]).'" -F "tipvin='.$row["tipvinext"].'" http://tecbib.com/coletaprodeca/lattes_xml_to_elastic.php');
    }
