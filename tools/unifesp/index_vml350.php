...
while (($row = oci_fetch_array($stid, OCI_ASSOC + OCI_RETURN_NULLS)) != false) {
    ...
    if (strlen($IDLattes) == 16) {
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, 'http://vml350.unifesp.br/import_lattes_to_elastic_dedup.php?lattesID=' . $IDLattes . '');
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, 'http://vml350.unifesp.br/import_lattes_to_elastic_dedup.php?lattesID=' . $row['COD_LATTES_16'] . '');
        curl_setopt($ch, CURLOPT_POSTFIELDS, implode('', $queryParams));
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        $output = curl_exec($ch);
