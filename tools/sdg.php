...
echo 'Total de registros faltantes: ' . $total . '';
echo '</br></br>';
...
foreach ($cursor["hits"]["hits"] as $record) {
    ...
    echo '<hr>';
    print_r($record['_source']['name']);
    echo '</br></br>';

    $json = file_get_contents('http://localhost:5000/api/v1/title?title=' . htmlentities(urlencode($record['_source']['name']), ENT_QUOTES) . '');
    $response_array = json_decode($json);
    rsort($response_array[1]);
    echo "<br/>";
    print_r($response_array[1][0]);
    echo "<br/>";
    $predict_number = round($response_array[1][0], 2);
    if ($predict_number < 0.75) {
        echo 'Não foi possível categorizar sua pergunta com confiabilidade.';
    } else {
