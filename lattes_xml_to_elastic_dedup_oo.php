...
if (isset($curriculo->{'PRODUCAO-BIBLIOGRAFICA'}->{'TRABALHOS-EM-EVENTOS'})) {
    ...
    foreach ($trabalhosEmEventosArray as $obra) {
        ...
        $doc_obj = new TrabalhosEmEventosLattes();
        $doc_obj->name = $dadosBasicosDoTrabalho['@attributes']["TITULO-DO-TRABALHO"];
        $doc_obj->lattes_ids[] = (string)$curriculo->attributes()->{'NUMERO-IDENTIFICADOR'};
        $doc_obj->datePublished = $dadosBasicosDoTrabalho['@attributes']["ANO-DO-TRABALHO"];
        $doc_obj->country = $dadosBasicosDoTrabalho['@attributes']["PAIS-DO-EVENTO"];
        $doc_obj->language = $dadosBasicosDoTrabalho['@attributes']["IDIOMA"];
        $doc_obj->pageStart = $detalhamentoDoTrabalho['@attributes']["PAGINA-INICIAL"];
        $doc_obj->pageEnd = $detalhamentoDoTrabalho['@attributes']["PAGINA-FINAL"];

        $doc_obj->publisher["organization"]["location"] = $detalhamentoDoTrabalho['@attributes']["CIDADE-DO-EVENTO"];
        $doc_obj->publisher["organization"]["name"] = $detalhamentoDoTrabalho['@attributes']["NOME-DA-EDITORA"];

        $doc_obj->lattes["natureza"] = $dadosBasicosDoTrabalho['@attributes']['NATUREZA'];
        $doc_obj->lattes["meioDeDivulgacao"] = $dadosBasicosDoTrabalho['@attributes']["MEIO-DE-DIVULGACAO"];
        $doc_obj->lattes["flagRelevancia"] = $dadosBasicosDoTrabalho['@attributes']["FLAG-RELEVANCIA"];
        $doc_obj->lattes["flagDivulgacaoCientifica"] = $dadosBasicosDoTrabalho['@attributes']["FLAG-DIVULGACAO-CIENTIFICA"];

        if (substr($dadosBasicosDoTrabalho['@attributes']["HOME-PAGE-DO-TRABALHO"], 0, 1 ) === "[") {
            $doc_obj->url = substr($dadosBasicosDoTrabalho['@attributes']["HOME-PAGE-DO-TRABALHO"], 1, -1);
        } else {
            $doc_obj->url = $dadosBasicosDoTrabalho['@attributes']["HOME-PAGE-DO-TRABALHO"];
        }

        if (Testadores::testDOI($dadosBasicosDoTrabalho['@attributes']["DOI"] === true)) {
            $doc_obj->doi = $dadosBasicosDoTrabalho['@attributes']["DOI"];
        }

        echo "<br/>";
        $obra_array["doc"] = (array) $doc_obj;
        print("<pre>".print_r($obra_array, true)."</pre>");
        echo "<br/>";

        echo "Array limpo:<br/>";
        print("<pre>".print_r(array_filter($obra_array["doc"]), true)."</pre>");
        ...
        echo "<br/>";
        $obra_array_doc = (array) $doc; 
        print("<pre>".print_r($obra_array_doc, true)."</pre>");
        echo "<br/>";
